...
const settingsMenuTitle = 'OKX Auto-opener'
...
const runCheck = () => {
  setTimeout(async () => {
    const iframe = document.querySelector('.web-app-body iframe')
    if(iframe) {
      isOpened = true
      const page = window.open(iframe.getAttribute('src'), '_blank')
      page.focus()
      await sleep(8000)
      page.close()
      await sleep(3000)
...
function closeApp() {
  clickElement(document.querySelector('.animated-close-icon'))
}
...
const HREF = 'https://web.telegram.org/k/#@OKX_official_bot'
...
async function scheduleAppOpen() {
  setTimeout(async () => {
    if (isGamePaused || isOpened) return;
